...
export default function AdminSidebar({ className = '', onClose, isOpen = false, refreshTrigger = 0 }: Props) {
  ...
  const nav = [
    /**
     * The four the mobile app's super-admin tab bar carries, in its order:
     * Hub, Admins, Events, Settings.
     *
     * `Approvals` and `Members` were separate entries here and are not tabs on
     * mobile — the Hub is where applications are browsed and a region is
     * staffed, so they are reached from there rather than duplicated in the nav.
     */
    { to: '/super-admin/dashboard', label: 'Hub', icon: <FaHome /> },
    { to: '/super-admin/admins', label: 'Admins', icon: <FaUserShield /> },
    { to: '/super-admin/events', label: 'Events', icon: <FaCalendarAlt /> },
