...
import { apiFetch, getAdminApplications, getAdminDashboard, toApplicationRecord, approveApplication, rejectApplication, errorMessage, type Applicant } from "@/services/activApi";
...
const Approvals = () => {
  ...
  /**
   * Load this tier's queue.
   *
   * The buckets are computed server-side — `classifyForLevel` decides what a
   * State Admin may act on — so there is no client-side status filtering here.
   * The filter this replaces matched `pending_state_approval`, a spelling this
   * backend does not use, so it selected nothing and the queue looked empty.
   */
  const load = async () => {
    try {
  ...
  const handleReject = async (id: string) => {
    try {
      ...
      const response = await apiFetch(`/applications/${id}/reject`, {
        ...
        body: JSON.stringify({ rejectionReason: 'Rejected by state admin' }),
      });
